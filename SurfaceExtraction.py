...
def make_octahedral_subgraph(voxel, weight):
    ...
    for direction in Grid.neighbors:
        ...
        G.add_node(tuple(voxel + direction * 0.5))

    for a, b in Grid.edges:
        G.add_edge(tuple(voxel + a), tuple(voxel + b), capacity=weight)
...
def generate_graph(v_crust, phi, v_int, v_ext, s=4, a=10**(-5)):
    ...
    for voxel in v_crust.get_voxels():
        ...
        G = nx.compose(subgraph, G)  # join graphs
        # connect to sink and source respectively
        for direction in Grid.neighbors:
            face = voxel + direction
            if face in v_ext or not v_crust.is_valid(face):
                G.add_edge("sink", tuple(voxel + direction * 0.5), capacity=weight)

            if face in v_int:
                G.add_edge("source", tuple(voxel + direction * 0.5), capacity=weight)
    return G
    ...
    for cutted_edge in cutset:
        for a, b in Grid.edges:
            if np.array_equal(np.array(cutted_edge[0]) - a*0.5, np.array(cutted_edge[1]) - b*0.5):
                S_opt.update(np.array(cutted_edge[0]) - a*0.5)
                break
    return S_opt
