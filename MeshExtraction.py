...
def get_neighbor_voxel(v, edge, center):
    ...
    vec = sign(np.prod(center, vec)) * vec
...
def get_center_adjacent_edges(voxel, center):
    ...
    vec = center - voxel
    x = tuple(voxel_center + np.dot(vec, np.array([1, 0, 0])) * np.array([1, 0, 0]))
    y = tuple(voxel_center + np.dot(vec, np.array([0, 1, 0])) * np.array([0, 1, 0]))
    z = tuple(voxel_center + np.dot(vec, np.array([0, 0, 1])) * np.array([0, 0, 1]))
    return [(x, y), (x, z), (y, z)]
...
def pred_intersect(s_opt, b):
    return len(s_opt.intersect(b)) >= 3
